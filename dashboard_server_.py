...
# ─── SSH via system SSH (no Paramiko) ─────────────────────────────────────────
def ssh_run(group: dict, command: str, timeout: int = 30) -> tuple:
    """Run a command on a remote server using system SSH — no Paramiko."""
    try:
        key_path = group.get("ssh_key", "")
        host = group["server_ip"]
        user = group["ssh_user"]
        ...
        return "", "ssh command not found — is OpenSSH installed?", False
    except Exception as e:
        return "", str(e), False


def check_process(group: dict, process_name: str) -> str:
    out, err, ok = ssh_run(group, f"pgrep -f '{process_name}' && echo running || echo stopped", timeout=10)
    if not ok:
        return "error"
...
def check_docker(group: dict) -> str:
    out, err, ok = ssh_run(group, "sudo docker ps | grep bazaar_lobby && echo running || echo stopped", timeout=10)
    if not ok:
        return "error"
    return "running" if "running" in out else "stopped"
...
# ─── BACKGROUND STATUS POLLING (reduced to 1 SSH call per group) ──────────────
def poll_group_status(group: dict) -> dict:
    """Run all status checks for one group in a single SSH session."""
    gid = group["id"]
    try:
        # Combine all checks into ONE ssh call to avoid rate limiting
        combined_cmd = (
            "echo '=SERVER=' && pgrep -f 'server.py' && echo running || echo stopped; "
            "echo '=AGENT=' && pgrep -f 'MAIAgentOperation' && echo running || echo stopped; "
            "echo '=DOCKER=' && sudo docker ps 2>/dev/null | grep -q bazaar_lobby && echo running || echo stopped; "
            "echo '=LOG=' && tail -n 3 ~/MAI2.0/logs/server.log 2>/dev/null || echo 'No logs'"
        )
        out, err, ok = ssh_run(group, combined_cmd, timeout=15)
        ...
        def parse_status(key):
            lines = sections.get(key, [])
            text = ' '.join(lines)
            return "running" if "running" in text else "stopped"

        return {
            "server_py": parse_status("SERVER"),
            "mai_agent": parse_status("AGENT"),
            "docker": parse_status("DOCKER"),
            "last_log": '\n'.join(sections.get("LOG", ["No logs"]))
        }
...
async def poll_status():
    """Poll each group one at a time with a delay to avoid overwhelming servers."""
    while True:
        for group in GROUPS:
            gid = group["id"]
            # Run in executor so it doesn't block the event loop
            status = await asyncio.get_running_loop().run_in_executor(
                None, poll_group_status, group
            )
...
async def handle_command(data: dict, ws: WebSocket):
    ...
    if action == "start_docker":
        ...
        def run_all():
            ...
            for cmd, success_msg, fail_msg in steps:
                out, err, ok = ssh_run(group, cmd, timeout=60)
                if success_msg:
                    msg = success_msg if ok else f"{fail_msg}: {err}"
                    safe_send(ws, {"type": "log", "group_id": group_id, "message": msg})
